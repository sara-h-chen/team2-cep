    ...
    function(data) {
		...
		function(data2) {
			...
			for(var j=0; j<scouts.length; ++j)
			{
			    ...
			    {
				$("#manualMatchTable").append("<tr id='MM"+scouts[j].id+"'><td>"+scouts[j].id+"</td><td>Scout "+scouts[j].id+"'s forename</td><td>Scout "+scouts[j].id+"'s surname</td><td>"+manualMatches[i].payment_description+"</td><td><button type='button' onclick='deleteMM("+scouts[j].id+");'>Delete Match</button></td></tr>");
				break;
			    }
			}
		    }
		...
		function(data)
		{
			...
			for(var i=0; i<scouts.length; ++i)
			{
				...
				var appendString = "<tr onmouseover='showScoutPayments("+scouts[i]['id']+")'><td>"+scouts[i]['id']+"</td><td>Scout "+scouts[i]['id']+" forename</td><td>Scout "+scouts[i]['id']+" surname</td>";
				for(var j=0; j<6; ++j)
				{
				...
				appendString += '<td><a href="mailto:?subject=Missing%20Payment(s)%20-%20Framwellgate%20Moor%20Scout%20Group&body=Dear%20Parent%2FGuardian%20of%20(Scout name here)%0A%0AOur%20records%20show%20that%20we%20have%20not%20received%20any%20payments%20from%20you%20in%20the%20past%20few%20months.%20Please%20could%20you%20check%20and%20ensure%20that%20these%20payments%20are%20made%20as%20soon%20as%20possible.%0A%0AWith%20thanks%2C%0A%0AJohn">Email</a></td>';
				appendString += "<td><button onclick='onClickRecord("+scouts[i]['id']+")'>Add Payment</button></td>";
				appendString += "</tr>";
				currentDate.setMonth(currentDate.getMonth()-6);
				var j=0;
...
$.get("http://community.dur.ac.uk/sara.h.chen/team2-cep/backend.php?table=saved", function(data)
{
	...
	{
		$("#savedScoutTable").append("<tr><td>"+data[i].id+"</td><td>Scout "+data[i].id+" forename</td><td>Scout "+data[i].id+" surname</td><td>"+data[i].reason+"</td><td><button onclick='deleteFlag("+data[i].id+");'>Delete Flag</button></td></tr>");
	}
});
...
function match(){
            ...
            if (score >= 5) {
                for (var k = 0; k<scouts.length; ++k) {
                    if (scouts[k].id == manualMatches[i].scout_id) {
                        ...
                        $("#matchingTable").append("<tr><td>Scout "+match.id+" forename</td><td>Scout "+match.id+" surname</td><td>"+match.payment_amount+"</td><td>"+match.payment_date+"</td><td>"+payments[j].description+"</td></tr>");
    ...
    for (var i=0; i<scouts.length; ++i) {
        ...
        $("#scoutUnmatchedTable").append("<tr id='sid" + scouts[i].id + "'><td>Scout "+scouts[i].id+" forename</td><td>Scout "+scouts[i].id+" surname</td><td><input type='radio' name='scout' value='sid"+scouts[i].id+"' /></td><td><button onclick='flagScout("+scouts[i].id+")'>Flag</button></td></tr>");
    }
